...
def init_db():
 ...
 # RESELLER KEY - Tera wala, ab isme 500 Rs hamesha rahega
 r=c.execute("SELECT * FROM users WHERE username='SAKSHAM11'").fetchone()
 if not r:
  c.execute("INSERT INTO users VALUES (NULL,'SAKSHAM11','123',500,0)")
 else:
  # Agar pehle se hai to balance 0 hai to 500 kar do
  if r['balance'] <= 0:
   c.execute("UPDATE users SET balance=500 WHERE username='SAKSHAM11'")
...
def register():
 ...
 if request.method=="POST":
  ...
  try:
   ...
   c.close();flash("User Already Exists")
...
@app.get("/admin")
def admin_panel():
 if not session.get('is_admin'): return "Admin only - Login with admin/admin123",403
